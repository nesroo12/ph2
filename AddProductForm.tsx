...
const AddProductForm: React.FC<AddProductFormProps> = ({ onAddProduct }) => {
  ...
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    type: 'Pharmaceutique' as Product['type'],
    pharmacyLocation: '',
    pharmacyComment: '',
    price: 0,
    inStock: true,
    prescribedBy: '',
    pharmacyId: '', // Will be set when submitting
    imageUrl: ''
  });
    ...
    return (
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className="bg-blue-600 text-white p-4 rounded-full shadow-lg hover:bg-blue-700 hover:shadow-xl transition-all duration-200 hover:scale-105 group"
        >
          <Plus className="h-6 w-6" />
          <div className="absolute right-full mr-3 top-1/2 -translate-y-1/2 bg-gray-900 text-white px-3 py-1 rounded-lg text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            Ajouter un produit
          </div>
        </button>
      </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-green-100">
        <div className="flex items-center justify-between p-6 border-b border-green-100 bg-gradient-to-r from-green-50 to-blue-50">
          <div>
            <h2 className="text-2xl font-bold text-green-800">Ajouter un produit</h2>
            <p className="text-green-600 mt-1">Remplissez les informations de votre médicament</p>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg p-4 mb-6">
            <div className="flex items-center gap-2 text-green-800 mb-2">
              ...
              <span className="font-medium">Information pour les pharmacies</span>
            </div>
            <p className="text-green-700 text-sm">
              Ajoutez vos médicaments disponibles pour permettre aux patients de les trouver facilement. 
